...
class DiscordConfig:
    def __init__(self, config: dict = {}):
        ...
        if "channels" not in config:
            raise ValueError("config lacks supported channels, bot will not do much")
    ...
    def get_token_rate(self, user: int, channel_id: int) -> float:
        ...
        if rate is None:
            rate = self.check_dict_try_get("default", "token_rate", channel_dict)
        
        # check if global default rate is set
        if rate is None:
            rate = self.check_dict_try_get("default", "token_rate", self.config)
