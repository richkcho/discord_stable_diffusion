...
import discord
from aioprocessing import AioEvent, AioQueue
...
class StableDiffusionDiscordClient(discord.Client):
    ...
    async def send_responses(self):
        ...
        while True:
            ...
            if len(work_item.images) == 0:
                await self.send_reply(message, f"Error handling request. Reason: {work_item.error_message}")
                continue

            pictures = work_item.images

            need_spoiler_tag = self.config.channel_requires_spoiler_tag(
                message.channel.id)
            if need_spoiler_tag:
    ...
    def process_message(self, message: discord.Message):
        ...
        def parse_config_value(name: str, request: dict, config: dict):
            ...
            value = ignore_exception(ValueError, config["default"])(
                config["type"])(request[name])
        ...
        def handle_set_config(request: dict, author_id: int, preferences: UserPreferences, response: List[str]):
            for config_name, config in PARAM_CONFIG.items():
                ...
                response.append("Setting default %s to %s" %
        ...
        if PREFIX in request:
            ...
            self.preferences.set_preference(
                message.author.id, PREFIX, request[PREFIX])
        if NEG_PREFIX in request:
            response.append("Setting negative prompt prefix as: %s" %
                            request[NEG_PREFIX])
            self.preferences.set_preference(
                message.author.id, NEG_PREFIX, request[NEG_PREFIX])
        if GET_PREFIX in request:
            response.append("Current prefix: %s" % self.preferences.get_preference(
                message.author.id, PREFIX))
        if GET_NEG_PREFIX in request:
            response.append("Current neg-prefix: %s" %
                            self.preferences.get_preference(message.author.id, NEG_PREFIX))
        if GET_MODELS in request:
            response.append("Supported models: %s" % str(
                PARAM_CONFIG[MODEL]["supported_values"]))
        if GET_VAES in request:
            response.append("Supported vaes: %s" % str(
                PARAM_CONFIG[VAE]["supported_values"]))
        ...
        if GET_EMBEDDINGS in request:
            for embedding in EMBEDDINGS:
                response.append(
                    "embedding name: %s, trigger words %s" % embedding)

        # parse setting defaults, and set defaults
        handle_set_config(request, message.author.id,
        ...
        if PROMPT in request or NEG_PROMPT in request or RAW_PROMPT in request or RAW_NEG_PROMPT in request:
            ...
            if RAW_PROMPT in request:
                ...
                prompt = make_prompt(
                    request, PROMPT, self.preferences.get_preference(message.author.id, PREFIX))

            if RAW_NEG_PROMPT in request:
                neg_prompt = request[RAW_NEG_PROMPT]
            else:
                neg_prompt = make_prompt(request, NEG_PROMPT, self.preferences.get_preference(
                    message.author.id, NEG_PREFIX))
            ...
            values = handle_config(
                request, message.author.id, self.preferences)
            ...
            response.append("Generating %d images for prompt: %s" %
                            (values[BATCH_SIZE], prompt))
            ...
            token_price = values[STEPS] * \
                values[WIDTH] * values[HEIGHT] / 262144.0
            if values[SCALE] > 1:
                response.append("Using highres upscaler %s, %d steps. Denoising str %.2f" % (
                    values[UPSCALER], values[HIGHRES_STEPS], values[DENOISING_STR]))
                token_price += 5 * \
                    values[HIGHRES_STEPS] * values[SCALE] * \
                    values[WIDTH] * values[HEIGHT] / 262144.0

            tokens = self.time_since_last_request(
                user) * self.get_token_rate(user, message.channel.id)
            ...
            response.append("Using model: %s, vae: %s, image size: %dx%d" % (
                values[MODEL], values[VAE], values[WIDTH], values[HEIGHT]))
            response.append("Using steps: %d, cfg: %.2f, sampler: %s, seed %d" % (
                values[STEPS], values[CFG], values[SAMPLER], values[SEED]))

            if self.work_queue.qsize() > 0:
                response.append("Your queue position is ~%d" %
